...
1. Leia `AGENTS.md`, `.sddharness/config.json` e os três docs atuais.
2. Rode `node scripts/docs-ready.mjs` se existir (só informativo neste passo).
3. **Detecte codebase útil** fora do skeleton do arnês:
   ...
   - Ignore: `node_modules/`, `.git/`, `feature_list.json`, `specs/`,
     `progress/`, `.sddharness/`, `.claude/`, `.cursor/`, `AGENTS.md`,
     `CLAUDE.md`, `CHECKPOINTS.md`, `init.sh` do arnês.
4. **Se o projeto estiver zerado / sem evidência suficiente:**
   - NÃO altere os três markdown.
   - Anote em `progress/current.md` que docs estão bloqueados.
